// SlovakForge App v8 — Listen mode, export, past tense
...
// Past tense parsing (field "past" in verb: "bol som, bol si, bol, boli sme, boli ste, boli")
function parsePast(past){if(!past)return[];return past.split(',').map((f,i)=>{const t=f.trim();return{sk:t,frPerson:FR_PERSONS[i]||'',full:t}})}
...
function exportVocab(){
  const data={version:3,lastUpdated:new Date().toISOString().split('T')[0],words:vocab};
  const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download='slovakforge-vocab-'+new Date().toISOString().split('T')[0]+'.json';
...
function makeDailyOpts(idx){
  ...
  // Decide: present conjugated, past, or plain
  let mode='plain';
  if(isVerb){
    const r=Math.random();
    if(hasPast&&r<0.3)mode='past';
    else mode='conj';
  }
  ...
  if(mode==='conj'){
    const form=randomConjForm(cor);
    if(!form){mode='plain';}
    else if(dir==='sk'){
      const corA=cor.fr+' ('+form.frPerson+')';
      const distA=dist.map(w=>{const v=(w._cat==='verb'||w.type==='verb')&&w.conjugation;if(v){const f=randomConjForm(w);return f?w.fr+' ('+f.frPerson+')':w.fr}return w.fr});
      dailyOpts=[...distA,corA].sort(()=>Math.random()-.5);
      dailyQcm[idx]._dir='sk';dailyQcm[idx]._prompt=form.full;dailyQcm[idx]._correct=corA;dailyQcm[idx]._mode='conj';dailyAns=null;return;
    }else{
      const corA=form.full;
      const distA=dist.map(w=>{const v=(w._cat==='verb'||w.type==='verb')&&w.conjugation;if(v){const f=randomConjForm(w);return f?f.full:(w.lemma||w.original)}return w.lemma||w.original});
      dailyOpts=[...distA,corA].sort(()=>Math.random()-.5);
      dailyQcm[idx]._dir='fr';dailyQcm[idx]._prompt=cor.fr+' ('+form.frPerson+')';dailyQcm[idx]._correct=corA;dailyQcm[idx]._mode='conj';dailyAns=null;return;
    }
  }

  if(mode==='past'){
    const form=randomPastForm(cor);
    if(!form){mode='plain';}
    else if(dir==='sk'){
      const corA=cor.fr+' pass\u00e9 ('+form.frPerson+')';
      const distA=dist.map(w=>{const hp=(w._cat==='verb'||w.type==='verb')&&w.past;if(hp){const f=randomPastForm(w);return f?w.fr+' pass\u00e9 ('+f.frPerson+')':w.fr}return w.fr});
      dailyOpts=[...distA,corA].sort(()=>Math.random()-.5);
      dailyQcm[idx]._dir='sk';dailyQcm[idx]._prompt=form.full;dailyQcm[idx]._correct=corA;dailyQcm[idx]._mode='past';dailyAns=null;return;
    }else{
      const corA=form.full;
      const distA=dist.map(w=>{const hp=(w._cat==='verb'||w.type==='verb')&&w.past;if(hp){const f=randomPastForm(w);return f?f.full:(w.lemma||w.original)}return w.lemma||w.original});
      dailyOpts=[...distA,corA].sort(()=>Math.random()-.5);
      dailyQcm[idx]._dir='fr';dailyQcm[idx]._prompt=cor.fr+' pass\u00e9 ('+form.frPerson+')';dailyQcm[idx]._correct=corA;dailyQcm[idx]._mode='past';dailyAns=null;return;
    }
  }

  // plain (non-verb or fallback)
  if(dir==='sk'){
    dailyOpts=[...dist.map(w=>w.fr),cor.fr].sort(()=>Math.random()-.5);
    dailyQcm[idx]._dir='sk';dailyQcm[idx]._prompt=cor.lemma||cor.original;dailyQcm[idx]._correct=cor.fr;dailyQcm[idx]._mode='plain';
  }else{
    dailyOpts=[...dist.map(w=>w.lemma||w.original),cor.lemma||cor.original].sort(()=>Math.random()-.5);
    dailyQcm[idx]._dir='fr';dailyQcm[idx]._prompt=cor.fr;dailyQcm[idx]._correct=cor.lemma||cor.original;dailyQcm[idx]._mode='plain';
  }
  dailyAns=null;
}
...
// Listen mode: auto-play audio when question appears
function playDailyAudio(){
  if(!dailyQcm||!dailyQcm[dailyIdx])return;
  const w=dailyQcm[dailyIdx];
  if(w._dir==='sk'){speak(w._prompt,'sk-SK')}
  else{speak(w._prompt,'fr-FR')}
}
...
if(dailyActive===true&&dailyQcm&&dailyQcm[dailyIdx]){
  ...
  if(listenMode&&dir==='sk'){
    // Listen mode: hide text, show ear + replay button
    h+='<div style="font-size:48px;margin:20px 0">\ud83d\udc42</div>';
    h+='<p class="muted" style="margin-bottom:10px">\u00c9coute et trouve la traduction</p>';
    h+='<button class="btn btn-sec" onclick="playDailyAudio()" style="margin-bottom:8px">\ud83d\udd04 R\u00e9\u00e9couter</button>';
    if(dailyAns!==null)h+='<h2 class="mono-b" style="font-size:22px;margin:8px 0;color:var(--acc)">'+esc(prompt)+'</h2>';
  }else{
    h+='<h2 class="mono-b" style="font-size:28px;margin:10px 0">'+esc(prompt)+'</h2>'+audioBtn(prompt,'6px 12px');
  }
  h+='</div>';
  ...
  if(listenMode&&dir==='sk'&&dailyAns===null)setTimeout(playDailyAudio,300);
  return;
}
...
if(allFlat().length>=4){
  ...
  h+='<div><span style="font-size:22px">\ud83e\udde0</span> <strong style="color:var(--purp);font-size:15px">Daily QCM</strong><br><span class="muted">Verbes conjugu\u00e9s + pass\u00e9 \u00b7 toutes cat\u00e9gories'+(dN>0?' \u00b7 <span style="color:#ff9800">'+dN+' dus</span>':'')+'</span></div>';
...
if(readLoading)h+='<div class="card" style="text-align:center;padding:30px"><div class="spinner" style="width:24px;height:24px;margin:0 auto 12px"></div><p class="muted">G\u00e9n\u00e9ration...</p></div>';
...
else fv.forEach(v=>{h+='<div class="card"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px"><span class="mono-b" style="font-size:17px">'+esc(v.lemma)+'</span>'+audioBtn(v.lemma,'4px 10px')+'<span class="muted" style="font-size:14px">= '+esc(v.fr)+'</span></div><p style="font-size:9px;color:var(--txD);margin-bottom:2px">Pr\u00e9sent</p>'+conjHTML(v.conjugation);if(v.past){h+='<p style="font-size:9px;color:#FF5722;margin-top:8px;margin-bottom:2px">Pass\u00e9</p>'+conjHTML(v.past)}if(v.example)h+='<p style="font-size:11px;color:var(--txM);font-style:italic;margin-top:10px">\ud83d\udcac '+esc(v.example)+'</p>';if(v.grammar_note)h+=grammarBadge(v.grammar_note);h+='</div>'});
h+='</div>';C.innerHTML=h;return;
}
}
