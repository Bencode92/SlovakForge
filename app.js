...
function trackReview(cat,lemma,ok){if(!cat||!vocab[cat])return;const i=vocab[cat].findIndex(x=>(x.lemma||x.original)===lemma);if(i===-1)return;const w=vocab[cat][i];w.reviews=(w.reviews||0)+1;if(!ok)w.errors=(w.errors||0)+1;w.box=ok?Math.min((w.box||0)+1,5):Math.max((w.box||0)-1,0);w.lastReview=Date.now();saveVocab()}
function getHardestWords(n){return allFlat().filter(w=>(w.reviews||0)>=2).sort((a,b)=>(b.errors||0)/(b.reviews||1)-(a.errors||0)/(a.reviews||1)).slice(0,n||10)}
function getGlobalStats(){const all=allFlat();const tr=all.reduce((s,w)=>s+(w.reviews||0),0);const te=all.reduce((s,w)=>s+(w.errors||0),0);return{total:all.length,learned:all.filter(w=>(w.box||0)>=3).length,due:getAllDueCount(),totalReviews:tr,totalErrors:te,rate:tr?Math.round((1-te/tr)*100):0}}
...
function errorRate(w){const r=w.reviews||0;if(r<2)return'';const e=w.errors||0;const pct=Math.round(e/r*100);if(pct<20)return'';return'<span class="tag" style="background:'+(pct>=50?'#E85D3A33':'#ff980033')+';color:'+(pct>=50?'#E85D3A':'#ff9800')+';font-size:9px">'+pct+'% err</span>'}

// ==================== RENDER ====================
function render(){
const C=$('content');if(!C)return;
const statsEl=$('header-stats');if(statsEl){const s=getGlobalStats();statsEl.textContent=s.total+' mots \u00b7 '+s.learned+' appris'+(s.due>0?' \u00b7 '+s.due+' dus':'')+(s.totalReviews>0?' \u00b7 '+s.rate+'%':'')}
