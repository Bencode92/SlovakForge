// SlovakForge App v5 — Daily QCM + all features
...
// Daily QCM state
let dailyQcm=null,dailyIdx=0,dailyAns=null,dailyScore={ok:0,total:0},dailyOpts=[];
...
function generateDailyPool(){
  const due=Object.values(vocab).flat().filter(isDue).sort(()=>Math.random()-.5).slice(0,10);
  const all=allFlat();
  const poolLemmas=new Set(due.map(w=>w.lemma||w.original));
  const extras=all.filter(w=>!poolLemmas.has(w.lemma||w.original)).sort(()=>Math.random()-.5).slice(0,15-due.length);
  return [...due,...extras].sort(()=>Math.random()-.5);
}
function startDailyQcm(){
  const pool=generateDailyPool();
  if(pool.length<4){readError='Pas assez de mots pour le Daily QCM (min 4).';render();setTimeout(()=>{readError='';render()},3000);return}
  dailyQcm=pool;dailyIdx=0;dailyAns=null;dailyScore={ok:0,total:0};
  learnMode='daily';makeDailyOpts(0);render();
}
function makeDailyOpts(idx){
  ...
  if(dir==='sk')dailyOpts=[...dist.map(w=>w.fr),cor.fr].sort(()=>Math.random()-.5);
  else dailyOpts=[...dist.map(w=>w.lemma||w.original),cor.lemma||cor.original].sort(()=>Math.random()-.5);
  dailyQcm[idx]._dir=dir;dailyAns=null;
}
function handleDailyAnswer(selected){
  const w=dailyQcm[dailyIdx];const dir=w._dir||'sk';
  ...
  render();
  setTimeout(()=>{if(dailyIdx+1<dailyQcm.length){dailyIdx++;makeDailyOpts(dailyIdx)}else{learnMode='daily-results'}render()},900);
}
...
function render(){
  ...
  if(currentTab==='read'){
    ...
    if(readText&&!readLoading){
      ...
      readText.sentences.forEach((s,i)=>{
        ...
        if(rev)h+='<div class="fr-translation">'+esc(s.fr)+'</div>';h+='</div>';
      });h+='</div>';
  ...
  if(currentTab==='learn'){
    ...
    if(!learnMode){
      // Daily QCM banner
      const totalW=allFlat().length;
      if(totalW>=4){
        h+='<div class="card" style="border-color:var(--purp);border-left:4px solid var(--purp);display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div><span style="font-size:20px">\ud83e\udde0</span> <strong style="color:var(--purp)">Daily QCM</strong><br><span class="muted">Quiz al\u00e9atoire toutes cat\u00e9gories m\u00e9lang\u00e9es</span></div><button class="btn" style="background:var(--purp);color:#fff;padding:10px 18px" onclick="startDailyQcm()">\ud83c\udfb2 Lancer</button></div>';
      }
      // Due words banner
    ...
    // DAILY QCM MODE
    else if(learnMode==='daily'&&dailyQcm&&dailyQcm[dailyIdx]){
      const w=dailyQcm[dailyIdx];const dir=w._dir||'sk';
      const prompt=dir==='sk'?(w.lemma||w.original):w.fr;
      const correct=dir==='sk'?w.fr:(w.lemma||w.original);
      const catInfo=CATS[w._cat||w.type]||{color:'#aaa',label:'?',icon:'?'};
      h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><button class="btn btn-sec" onclick="learnMode=null;render()">\u2190 Retour</button><span class="mono" style="font-size:11px;color:var(--txD)">Daily '+(dailyIdx+1)+'/'+dailyQcm.length+'</span></div>';
      ...
      h+='<div style="display:flex;justify-content:center;gap:6px;margin-bottom:10px"><span class="tag" style="background:'+catInfo.color+'22;color:'+catInfo.color+'">'+catInfo.icon+' '+catInfo.label+'</span><span class="tag" style="background:var(--brd);color:var(--txD)">'+(dir==='sk'?'SK \u2192 FR':'FR \u2192 SK')+'</span></div>';
      h+='<h2 class="mono-b" style="font-size:26px;margin:10px 0">'+esc(prompt)+'</h2>'+audioBtn(prompt,'6px 12px')+'</div>';
    ...
    // DAILY RESULTS
    else if(learnMode==='daily-results'){
      ...
      h+='<div style="display:flex;gap:10px;justify-content:center;margin-top:22px;flex-wrap:wrap"><button class="btn" style="background:var(--purp);color:#fff" onclick="startDailyQcm()">\ud83c\udfb2 Relancer</button><button class="btn btn-sec" onclick="learnMode=null;render()">\u2190 Menu</button></div></div>';
    }
