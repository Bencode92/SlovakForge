// SlovakForge API Layer v3 — Sprint 1: save queue, grammar_note, audio check
...
// === GitHub: READ via raw URL (UTF-8 safe) ===
...
// === GitHub: WRITE (UTF-8 safe via TextEncoder) ===
...
// === Save Queue (debounce to avoid 409 conflicts) ===
...
async function saveVocabToGH(vocabData){
  ...
  try{
    ...
    const data={version:2,lastUpdated:new Date().toISOString().split('T')[0],words:vocabData};
...
// === Audio — Web Speech API with sk-SK check ===
let _skVoiceAvailable=null;
function checkSkVoice(){
  if(!('speechSynthesis' in window)){_skVoiceAvailable=false;return}
  const check=()=>{const voices=speechSynthesis.getVoices();_skVoiceAvailable=voices.some(v=>v.lang&&v.lang.startsWith('sk'))};
  check();
  if(speechSynthesis.onvoiceschanged!==undefined)speechSynthesis.onvoiceschanged=check;
}
function speak(text,lang){
  if(!('speechSynthesis' in window))return;
  speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(text);
  u.lang=lang||'sk-SK';u.rate=0.85;u.pitch=1;
  // Try to find a Slovak voice explicitly
  const voices=speechSynthesis.getVoices();
  const skVoice=voices.find(v=>v.lang&&v.lang.startsWith('sk'));
  if(skVoice)u.voice=skVoice;
...
async function aiGenerateText(theme,level){
  ...
  const desc={easy:'Niveau FACILE (A1): phrases courtes (5-7 mots), present uniquement, mots tres courants.',medium:'Niveau MOYEN (A2-B1): phrases moyennes, present+passe, conjonctions simples (ale, pretože, keď).',hard:'Niveau DIFFICILE (B1-B2): phrases complexes, tous temps, subordonnees, expressions idiomatiques.'}[level];
...
async function aiTranslateWord(frenchWord){
  const sys='Tu traduis un mot francais en slovaque. Reponds UNIQUEMENT en JSON valide sans markdown sans backticks.\nFormat: {"original":"mot francais","lemma":"mot slovaque","type":"verb|noun|adjective|conjunction|expression","fr":"mot francais","gender":"M|F|N ou null","plural":"pluriel ou null","conjugation":"conjugaison present ou null","example":"phrase SK = FR","tip":"astuce mnemo","grammar_note":null}';
